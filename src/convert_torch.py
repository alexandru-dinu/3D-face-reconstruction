"""
Use with torch 0.4.x
original source: https://github.com/clcarwin/convert_torch_to_pytorch

TODO: Not Implemented nn.SpatialUpSamplingBilinear
"""
...
import torch
assert torch.__version__.startswith("0.4"), "Use with torch 0.4.x"
    ...
    for m in module.modules:
        ...
        if name == 'SpatialConvolution' or name == 'nn.SpatialConvolutionMM':
            if not hasattr(m, 'groups') or m.groups is None:
                m.groups = 1
            n = nn.Conv2d(m.nInputPlane, m.nOutputPlane, (m.kW, m.kH), (m.dW, m.dH),
                          (m.padW, m.padH), 1, m.groups, bias=(m.bias is not None))
            copy_param(m, n)
            add_submodule(seq, n)
        elif name == 'SpatialBatchNormalization':
            n = nn.BatchNorm2d(m.running_mean.size(
                0), m.eps, m.momentum, m.affine)
            copy_param(m, n)
            add_submodule(seq, n)
        elif name == 'VolumetricBatchNormalization':
            n = nn.BatchNorm3d(m.running_mean.size(
                0), m.eps, m.momentum, m.affine)
            copy_param(m, n)
            add_submodule(seq, n)
        elif name == 'ReLU':
            ...
            n = nn.MaxPool2d((m.kW, m.kH), (m.dW, m.dH),
                             (m.padW, m.padH), ceil_mode=m.ceil_mode)
            add_submodule(seq, n)
        elif name == 'SpatialAveragePooling':
            n = nn.AvgPool2d((m.kW, m.kH), (m.dW, m.dH),
                             (m.padW, m.padH), ceil_mode=m.ceil_mode)
            add_submodule(seq, n)
            ...
            n = nn.ConvTranspose2d(m.nInputPlane, m.nOutputPlane, (m.kW, m.kH),
                                   (m.dW, m.dH), (m.padW, m.padH), (m.adjW, m.adjH))
            copy_param(m, n)
            add_submodule(seq, n)
        elif name == 'VolumetricFullConvolution':
            n = nn.ConvTranspose3d(m.nInputPlane, m.nOutputPlane, (m.kT, m.kW, m.kH), (
                m.dT, m.dW, m.dH), (m.padT, m.padW, m.padH), (m.adjT, m.adjW, m.adjH), m.groups)
            copy_param(m, n)
            add_submodule(seq, n)
            ...
            print('Not Implemented', name, real._typename)
        else:
...
def lua_recursive_source(module):
    ...
    for m in module.modules:
        ...
        if name == 'SpatialConvolution' or name == 'nn.SpatialConvolutionMM':
            ...
            s += ['nn.BatchNorm3d({},{},{},{}),#BatchNorm3d'.format(
                m.running_mean.size(0), m.eps, m.momentum, m.affine)]
        elif name == 'ReLU':
            s += ['nn.ReLU()']
        elif name == 'Sigmoid':
            ...
            s += ['nn.ReplicationPad2d({})'.format((m.pad_l,
                                                    m.pad_r, m.pad_t, m.pad_b))]
        elif name == 'SpatialReflectionPadding':
            s += ['nn.ReflectionPad2d({})'.format((m.pad_l,
                                                   m.pad_r, m.pad_t, m.pad_b))]
        elif name == 'Copy':
            ...
            lrn = 'lnn.SpatialCrossMapLRN(*{})'.format((m.size,
                                                        m.alpha, m.beta, m.k))
...
class LambdaReduce(LambdaBase):
    ...
    varname = t7_filename.split("/")[-1].replace('.t7', '').replace('.', '_').replace('-', '_')
...
parser.add_argument('--model', type=str, required=True, help='torch model file in t7 format')
parser.add_argument('--out_py', type=str, required=True, help='output file name prefix, xxx.py')
parser.add_argument('--out_pth', type=str, required=True, help='output file name prefix, xxx.pth')
args = parser.parse_args()

torch_to_pytorch(args.model, args.out_py, args.out_pth)
