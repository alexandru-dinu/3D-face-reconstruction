...
def lua_recursive_model(module, seq):
    for m in module.modules:
        ...
        if name == "SpatialConvolution" or name == "nn.SpatialConvolutionMM":
            ...
            n = nn.MaxPool2d(
                (m.kW, m.kH), (m.dW, m.dH), (m.padW, m.padH), ceil_mode=m.ceil_mode
            )
            add_submodule(seq, n)
            ...
            n = nn.ConvTranspose3d(
                m.nInputPlane,
                m.nOutputPlane,
                (m.kT, m.kW, m.kH),
                (m.dT, m.dW, m.dH),
                (m.padT, m.padW, m.padH),
                (m.adjT, m.adjW, m.adjH),
                m.groups,
            )
            copy_param(m, n)
            add_submodule(seq, n)
...
def lua_recursive_source(module):
    ...
    for m in module.modules:
        ...
        if name == "SpatialConvolution" or name == "nn.SpatialConvolutionMM":
            ...
            s += [
                "nn.BatchNorm2d({},{},{},{}),#BatchNorm2d".format(
                    m.running_mean.size(0), m.eps, m.momentum, m.affine
                )
            ]
        elif name == "VolumetricBatchNormalization":
            s += [
                "nn.BatchNorm3d({},{},{},{}),#BatchNorm3d".format(
                    m.running_mean.size(0), m.eps, m.momentum, m.affine
                )
            ]
        elif name == "ReLU":
            ...
            s += [
                "nn.MaxPool2d({},{},{},ceil_mode={}),#MaxPool2d".format(
                    (m.kW, m.kH), (m.dW, m.dH), (m.padW, m.padH), m.ceil_mode
                )
            ]
        elif name == "SpatialAveragePooling":
            s += [
                "nn.AvgPool2d({},{},{},ceil_mode={}),#AvgPool2d".format(
                    (m.kW, m.kH), (m.dW, m.dH), (m.padW, m.padH), m.ceil_mode
                )
            ]
        elif name == "SpatialUpSamplingNearest":
            ...
            s += [
                "nn.ReplicationPad2d({})".format((m.pad_l, m.pad_r, m.pad_t, m.pad_b))
            ]
        elif name == "SpatialReflectionPadding":
            ...
            s += [
                "Lambda(lambda x,a={}: x.narrow(*a))".format(
...
parser.add_argument(
    "--model", type=str, required=True, help="torch model file in t7 format"
)
parser.add_argument(
    "--out_py", type=str, required=True, help="output file name prefix, xxx.py"
)
parser.add_argument(
    "--out_pth", type=str, required=True, help="output file name prefix, xxx.pth"
)
