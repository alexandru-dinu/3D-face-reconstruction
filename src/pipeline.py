...
optimizer = optim.Adam(learnable_params, lr=1e-4)
...
def train():
    ...
    for epoch in range(1, 410):  # loop over the dataset multiple times
        ...
        for i, data in enumerate(trainloader, start=1):
            ...
            running_avg = loss if i == 1 else running_avg * (i - 1) / i + loss / i
            ...
            if i % 1 == 0:
                print('[%2d, %5d/%5d] loss: %.8f, run_avg: %.8f' % (epoch, i, len(trainloader), running_loss / 1, running_avg))
                running_loss = 0.0
